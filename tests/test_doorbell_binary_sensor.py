...
class DoorbellBinarySensorTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_setup_keeps_non_doorbell_diagnostics_for_unsupported_control(self) -> None:
        ...
        self.assertTrue(
            any(
                isinstance(entity, binary_sensor_module.LoxoneDiagnosticBinaryEntity)
                and entity._state_name == "alarm"
                for entity in entities
            )
        )
